...
3Dモデル（GLB / glTF / FBX）をブラウザにドラッグ＆ドロップするだけで、Humanoidボーンの自動認識・VRM 1.0メタ情報の設定・表情（Expressions）の自動マッピング・VRM書き出しまでを行えるWebアプリです。3DやVRMの専門知識がなくても、数クリックでVRMアバターを作成できることを目指しています。
...
1. トップ画面に `.glb` / `.gltf` / `.fbx` ファイルをドラッグ＆ドロップ（またはクリックして選択）
...
- 入力: `.glb`, `.gltf`, `.fbx`（Skeleton/SkinnedMeshを含むモデルを想定）
- 出力: `.vrm`（VRM 1.0 / VRM 0.x）
- ボーンを持たないモデルも読み込み可能で、「AIで自動リギングする」から形状ベースのヒューリスティックによる自動ボーン生成を試せます（後述の「自動リギングの説明」参照。完全な精度は保証されないβ機能です）

### FBX入力について

`.fbx` は `three/examples/jsm/loaders/FBXLoader.js`（three.js本体に同梱、追加依存なし）でパースしています。GLTFLoaderと違い `FBXLoader.parse()` は同期APIで、シーングラフ自体は即座に返りますが埋め込みテクスチャのデコードは非同期に続くため、`ModelLoader.ts` の `waitForTextureImages()` で各テクスチャの `HTMLImageElement.complete` を個別に待ち合わせています。

BlenderやUnityですでにHumanoidリグ済みのFBXを書き出せば、そのボーン階層がそのまま読み込まれ、ボーンを持たないモデル用の「AIで自動リギングする」ヒューリスティック（`AutoRigger.ts`）を経由せず、既存の `BoneDetector` / `HumanoidMapper` によるボーン名からHumanoidへの自動マッピングだけで済みます。既にリグ済みのモデルをclusterなどのUnity Humanoidベースのプラットフォームに出す場合、こちらの経路の方が意図した関節の曲がり方に近い結果になりやすいです。

制約として、FBX内で外部ファイル参照になっているテクスチャ（同梱されていないもの）はブラウザ内では解決できません。埋め込みテクスチャ（Video/Contentノードに実データを含む形式）のみ読み込まれます。
...
その他、OBJ/VRM入力対応、アニメーションプレビュー、Lip Sync等は将来検討事項です。
